...
<!-- ANTI-FOUC: apply saved theme before CSS loads to prevent flash -->
<script>
(function(){
  try{
...
<style>
:root {
  --bg:#050d1a;--card-bg:rgba(0,255,255,0.04);--card-bg2:rgba(5,15,35,0.85);
  --border:rgba(0,255,255,0.12);--border2:rgba(0,255,255,0.08);
  --text-primary:#e0f7ff;--text-secondary:rgba(200,230,255,0.75);
  --text-muted:rgba(0,255,255,0.50);--text-dim:rgba(180,210,255,0.60);
  --acc:#00ffff;--ap:#4facfe;
  --topbar-bg:rgba(0,255,255,0.05);--row-hover:rgba(0,255,255,0.04);
  --thead-bg:rgba(79,172,254,0.06);--input-bg:rgba(0,0,0,0.40);
  --input-border:rgba(0,255,255,0.20);--pbar-bg:rgba(255,255,255,0.08);
  --grp-head-bg:rgba(79,172,254,0.10);--ri-bg:rgba(0,0,0,0.15);
  --modal-bg:rgba(5,15,35,0.97);
  --sec-title-color:#00ffff;--card-title-color:rgba(0,255,255,0.50);
  --grp-name-color:#00ffff;--ri-title-color:#00ffff;--ci-name-color:#00ffff;
  --score-val-color:#00ffff;--chart-grid:rgba(0,255,255,0.04);--gold-text:#ffd32a;
  --title-color:#00ffff;
...
[data-theme="light"] {
  --bg:#f0f4ff;--card-bg:#ffffff;--card-bg2:#f8faff;
  --border:rgba(79,100,220,0.20);--border2:rgba(79,100,220,0.14);
  --text-primary:#0d1333;--text-secondary:#1e2a5e;
  --text-muted:rgba(50,70,180,0.65);--text-dim:rgba(30,42,94,0.72);
  --acc:#1a3bbf;--ap:#2563eb;
  --topbar-bg:#ffffff;--row-hover:rgba(79,100,220,0.05);
  --thead-bg:rgba(79,100,220,0.07);--input-bg:#f5f8ff;
  --input-border:rgba(79,100,220,0.25);--pbar-bg:rgba(0,0,0,0.08);
  --grp-head-bg:rgba(37,99,235,0.08);--ri-bg:rgba(79,100,220,0.04);
  --modal-bg:#ffffff;
  --sec-title-color:#1a3bbf;--card-title-color:rgba(50,70,180,0.65);
  --grp-name-color:#1a3bbf;--ri-title-color:#1a3bbf;--ci-name-color:#1a3bbf;
  --score-val-color:#1a3bbf;--chart-grid:rgba(79,100,220,0.08);
  --gold-text:#8a6500;--title-color:#1a3bbf;
...
body,html{height:100%;font-family:'Rajdhani',sans-serif;background:var(--bg);color:var(--text-primary);overflow:hidden;transition:background .3s,color .3s;}
#particles-js{position:fixed;width:100%;height:100%;top:0;left:0;z-index:1;}
.layout{display:flex;height:100vh;position:relative;z-index:10;}

/* SIDEBAR */
.sidebar{width:230px;min-width:230px;flex-shrink:0;background:var(--sb);display:flex;flex-direction:column;padding:18px 12px;height:100vh;overflow-y:auto;border-right:1px solid var(--sb-bdr);z-index:20;transition:background .3s,border-color .3s;}
...
.topbar{display:flex;justify-content:space-between;align-items:center;background:var(--topbar-bg);border:1px solid var(--border);padding:10px 15px;border-radius:12px;margin-bottom:18px;gap:10px;transition:background .3s;}
...
.logout-btn{text-decoration:none;background:#ff4757;color:#fff;padding:7px 12px;border-radius:8px;font-size:.8rem;font-weight:700;flex-shrink:0;transition:.2s;}
.logout-btn:hover{background:#c0392b;}
.settings-btn{background:rgba(79,172,254,.14);border:1px solid rgba(79,172,254,.30);color:#4facfe;padding:7px 12px;border-radius:8px;font-size:.8rem;cursor:pointer;font-family:'Rajdhani',sans-serif;font-weight:700;display:inline-flex;align-items:center;gap:5px;transition:.2s;flex-shrink:0;}
...
.sec-title{font-family:'Orbitron',sans-serif;font-size:.88rem;letter-spacing:2px;color:var(--sec-title-color);margin-bottom:14px;padding-bottom:9px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:9px;}
...
.stat-card{background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:14px 10px;text-align:center;transition:transform .2s,background .3s;}
...
/* CARD */
.card{background:var(--card-bg);border:1px solid var(--border2);border-radius:13px;padding:16px;margin-bottom:18px;overflow-x:auto;transition:background .3s;}
...
/* Activities table — status column right-aligned to match badge */
.grp-body table th:last-child,
.grp-body table td:last-child{text-align:right;}
...
/* SEARCH */
.search-bar{padding:8px 11px;border-radius:8px;border:1px solid var(--input-border);background:var(--input-bg);color:var(--text-primary);font-family:'Rajdhani',sans-serif;font-size:.88rem;width:250px;max-width:100%;outline:none;transition:border .2s,background .3s;}
.search-bar:focus{border-color:#4facfe;}
.search-bar::placeholder{color:var(--text-muted);}
.search-btn{padding:8px 14px;border-radius:8px;border:none;background:linear-gradient(135deg,#4facfe,#00c6ff);color:#050d1a;font-family:'Rajdhani',sans-serif;font-weight:700;font-size:.86rem;cursor:pointer;}
...
.class-item{background:var(--card-bg);border:1px solid var(--border);border-radius:12px;padding:14px;transition:transform .2s,background .3s;}
...
.grp-head{background:var(--grp-head-bg);padding:12px 16px;display:flex;align-items:center;gap:10px;border-bottom:1px solid var(--border);flex-wrap:wrap;}
...
/* REMINDERS */
.ri{background:var(--ri-bg);border:1px solid var(--border2);border-radius:10px;padding:12px 14px;margin-bottom:8px;}
...
/* SCORE CARD */
.score-card{background:rgba(79,172,254,.06);border:1px solid var(--border);border-radius:16px;padding:22px 24px;margin-bottom:18px;display:flex;align-items:center;gap:24px;flex-wrap:wrap;transition:background .3s;}
.score-circle{width:90px;height:90px;border-radius:50%;border:4px solid #4facfe;display:flex;flex-direction:column;align-items:center;justify-content:center;flex-shrink:0;background:rgba(79,172,254,.08);}
...
.alert a{margin-left:auto;padding:6px 12px;border-radius:8px;text-decoration:none;font-size:.8rem;font-weight:700;flex-shrink:0;background:rgba(255,211,42,.20);color:#e6b800;border:1px solid rgba(255,211,42,.30);}
...
/* REMINDER PREVIEW */
.preview-ri{border-radius:10px;padding:12px 15px;margin-bottom:9px;border:1px solid rgba(255,211,42,.22);background:rgba(255,211,42,.05);}
...
/* MODAL */
.modal-overlay{display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.65);backdrop-filter:blur(5px);align-items:center;justify-content:center;}
.modal-overlay.open{display:flex;}
.modal-box{background:var(--modal-bg);border:1px solid var(--border);border-radius:18px;padding:28px 30px;width:90%;max-width:360px;box-shadow:0 24px 64px rgba(0,0,0,.5);}
...
.theme-btn{padding:11px;border:2px solid var(--border);border-radius:10px;background:transparent;color:var(--text-secondary);font-family:'Rajdhani',sans-serif;font-weight:700;font-size:.88rem;cursor:pointer;transition:.2s;display:flex;align-items:center;justify-content:center;gap:7px;}
.theme-btn:hover,.theme-btn.on{border-color:#4facfe;background:rgba(79,172,254,.12);color:var(--title-color);}
...
<div class="sidebar">
  ...
  <a href="#" data-sec="overview" class="<?php echo $active_section=='overview'?'active':''; ?>">
    <i class="fas fa-tachometer-alt"></i> Overview
  </a>
  <a href="#" data-sec="classes" class="<?php echo $active_section=='classes'?'active':''; ?>">
    <i class="fas fa-chalkboard"></i> My Classes <span class="bc"><?php echo $total_classes; ?></span>
  </a>

  <span class="sl">Academic</span>
  <a href="#" data-sec="grades" class="<?php echo $active_section=='grades'?'active':''; ?>">
    <i class="fas fa-graduation-cap"></i> Grades
  </a>
  <a href="#" data-sec="activities" class="<?php echo $active_section=='activities'?'active':''; ?>">
    <i class="fas fa-tasks"></i> Activities
    <?php if($acts_pending>0): ?><span class="bc red"><?php echo $acts_pending; ?></span><?php endif; ?>
  </a>
  <a href="#" data-sec="reminders" class="<?php echo $active_section=='reminders'?'active':''; ?>">
    <i class="fas fa-bell"></i> Reminders
    <?php if($total_reminders>0): ?><span class="bc"><?php echo $total_reminders; ?></span><?php endif; ?>
  </a>
  <a href="#" data-sec="performance" class="<?php echo $active_section=='performance'?'active':''; ?>">
    <i class="fas fa-chart-line"></i> My Performance
  </a>
  <a href="#" data-sec="analytics" class="<?php echo $active_section=='analytics'?'active':''; ?>">
...
<!-- ════════ OVERVIEW ════════ -->
<div class="section <?php echo $active_section=='overview'?'active':''; ?>" data-sec="overview">
  ...
  <div class="alert alert-gold">
    ...
    <a href="#" data-sec="activities">View <i class="fas fa-arrow-right"></i></a>
  </div>
  <?php endif; ?>
  ...
  <a href="#" data-sec="reminders" style="display:inline-flex;align-items:center;gap:6px;color:var(--ap);font-size:.82rem;text-decoration:none;margin-bottom:16px;"><i class="fas fa-arrow-right"></i> See all reminders</a>
  <?php endif; ?>
</div>

<!-- ════════ MY CLASSES ════════ -->
<div class="section <?php echo $active_section=='classes'?'active':''; ?>" data-sec="classes">
...
<!-- ════════ GRADES ════════ -->
<div class="section <?php echo $active_section=='grades'?'active':''; ?>" data-sec="grades" id="grades-section">
  ...
  <form method="GET" style="margin-bottom:14px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <input type="text" id="gradeSearch" class="search-bar" placeholder="Search subject..." oninput="filterGrades()">
      <button type="button" class="search-btn" onclick="filterGrades()"><i class="fas fa-search"></i> Search</button>
    </div>
  </form>
...
<!-- ════════ ACTIVITIES ════════ -->
<div class="section <?php echo $active_section=='activities'?'active':''; ?>" data-sec="activities">
  ...
  <div class="grp-block">
    ...
    <div class="grp-body">
      <table>
        <tr><th style="width:75%;">Activity</th><th style="width:25%;text-align:right;">Status</th></tr>
...
<!-- ════════ REMINDERS ════════ -->
<div class="section <?php echo $active_section=='reminders'?'active':''; ?>" data-sec="reminders">
  ...
  <div class="grp-block">
    ...
    <div style="padding:10px 14px;">
      ...
      <div class="<?php echo $ri_cls; ?>">
        ...
        <div class="ri-meta">
          <?php if(!empty($row['due_date'])): ?><span style="<?php echo $overdue?'color:#ff6b81;':''; ?>"><i class="fas fa-calendar-alt"></i> Due: <?php echo date('M j, Y',strtotime($row['due_date'])); ?></span><?php endif; ?>
...
<!-- ════════ PERFORMANCE ════════ -->
<div class="section <?php echo $active_section=='performance'?'active':''; ?>" data-sec="performance">
    ...
    <?php else:
      foreach($grades_arr as $row):
        $avg_d=($row['prelim']>0||$row['midterm']>0||$row['final']>0)
               ?round($row['prelim']*0.25+$row['midterm']*0.25+$row['final']*0.50,2):0;
...
<!-- ════════ ANALYTICS ════════ -->
<div class="section <?php echo $active_section=='analytics'?'active':''; ?>" data-sec="analytics">
  ...
  <div class="chart-grid">
    <div class="chart-card"><h3><i class="fas fa-chart-bar"></i> Grade Average per Subject</h3><canvas id="an_avgBar" height="220"></canvas></div>
    <div class="chart-card"><h3><i class="fas fa-chart-pie"></i> Pass vs Fail</h3><canvas id="an_passFail" height="220"></canvas></div>
    <div class="chart-card"><h3><i class="fas fa-tasks"></i> Activity Completion per Subject</h3><canvas id="an_actBar" height="220"></canvas></div>
    <div class="chart-card"><h3><i class="fas fa-chart-line"></i> Prelim / Midterm / Final Breakdown</h3><canvas id="an_grouped" height="220"></canvas></div>
  </div>
</div>
...
<script>
particlesJS('particles-js',{particles:{number:{value:65,density:{enable:true,value_area:900}},color:{value:"#4facfe"},shape:{type:"circle"},opacity:{value:0.22,random:true},size:{value:2.1,random:true},line_linked:{enable:true,distance:130,color:"#00c6ff",opacity:0.12,width:1},move:{enable:true,speed:1.2,direction:"none",random:true}},interactivity:{detect_on:"canvas",events:{onhover:{enable:true,mode:"grab"},onclick:{enable:true,mode:"push"}},modes:{grab:{distance:130,line_linked:{opacity:0.35}},push:{particles_nb:3}}},retina_detect:true});
</script>
...
function applyAccent(c){
  R.style.setProperty('--ap',c);localStorage.setItem('paops_accent',c);
...
/* Sync buttons — theme already applied by <head> anti-FOUC script */
(function(){
  applyTheme(localStorage.getItem('paops_theme')||'dark');
  applyAccent(localStorage.getItem('paops_accent')||'#4facfe');
})();

/* ── SECTION SWITCHER — no page reload ── */
function showSection(name){
  document.querySelectorAll('.section').forEach(s=>s.classList.remove('active'));
  document.querySelectorAll('.sidebar a[data-sec]').forEach(a=>a.classList.remove('active'));
  const sec=document.querySelector('.section[data-sec="'+name+'"]');
  const lnk=document.querySelector('.sidebar a[data-sec="'+name+'"]');
  if(sec)sec.classList.add('active');
  if(lnk)lnk.classList.add('active');
  document.querySelector('.main').scrollTop=0;
  history.replaceState(null,'','?section='+name);
}
// Live grade search — filters blocks by class name
/* ── SECTION SWITCHER clicks ── */
...
function filterGrades(){
  ...
  document.querySelectorAll('#grades-section .grp-block').forEach(function(block){
    var nm=block.querySelector('.grp-name');
    if(!nm) return;
    block.style.display=(!q||nm.textContent.toLowerCase().includes(q))?'':'none';
  });
}
/* ── CHARTS ── */
Chart.defaults.color=chartLabel();
...
function mkBar(id,labels,data,color){
  ...
  const gc=chartGrid(),lc=chartLabel();
  new Chart(el,{type:'bar',data:{labels,datasets:[{data,backgroundColor:color+'88',borderColor:color,borderWidth:2,borderRadius:5}]},options:{responsive:true,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:gc},ticks:{color:lc}},x:{grid:{color:gc},ticks:{color:lc}}}}});
}
function mkGrouped(id,labels,datasets){
  const el=document.getElementById(id);if(!el)return;
  const gc=chartGrid();
  new Chart(el,{type:'bar',data:{labels,datasets},options:{responsive:true,plugins:{legend:{position:'bottom',labels:{padding:10,boxWidth:10,color:chartLabel()}}},scales:{y:{beginAtZero:true,max:100,grid:{color:gc},ticks:{color:chartLabel()}},x:{grid:{color:gc},ticks:{color:chartLabel()}}}}});
}
function mkGroupedStack(id,labels,datasets){
  const el=document.getElementById(id);if(!el)return;
  const gc=chartGrid(),lc=chartLabel();
  new Chart(el,{type:'bar',data:{labels,datasets},options:{responsive:true,plugins:{legend:{position:'bottom',labels:{padding:10,boxWidth:10,color:lc}}},scales:{x:{stacked:true,grid:{color:gc},ticks:{color:lc}},y:{stacked:true,beginAtZero:true,grid:{color:gc},ticks:{color:lc}}}}});
}
function mkDonut(id,labels,data,colors){
  const el=document.getElementById(id);if(!el)return;
  new Chart(el,{type:'doughnut',data:{labels,datasets:[{data,backgroundColor:colors.map(c=>c+'99'),borderColor:colors,borderWidth:2}]},options:{responsive:true,plugins:{legend:{position:'bottom',labels:{padding:10,boxWidth:10,color:chartLabel()}}},cutout:'58%'}});
}
